...
class Coupon_Automation_Security
{
    ...
    private function sanitize_api_key($api_key)
    {
        ...
        // More permissive validation for API keys (includes base64 characters)
        if (!preg_match('/^[a-zA-Z0-9_\-\.\/\+=]+$/', $sanitized)) {
            return '';
        }

        // Additional length validation (most API keys are between 20-200 characters)
        if (strlen($sanitized) < 10 || strlen($sanitized) > 500) {
            return '';
        }
