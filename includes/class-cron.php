...
class Coupon_Automation_Cron {
    ...
    private function check_api_configuration() {
        ...
        foreach ($critical_apis as $api_key => $display_name) {
            $key_value = $this->settings ? $this->settings->get_api_key($api_key) : '';
            if (empty($key_value)) {
                $issues[] = "Missing API key: {$display_name}";
            }
        }
        
        // Check AWIN Publisher ID specifically
        $awin_publisher_id = $this->settings ? $this->settings->get_api_key('awin_publisher_id') : '';
        $awin_api_token = $this->settings ? $this->settings->get_api_key('awin_api_token') : '';
        
        if (!empty($awin_api_token) && empty($awin_publisher_id)) {
            $issues[] = 'AWIN API token provided but Publisher ID is missing';
        }
